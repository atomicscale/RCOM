...
int llwrite() {
	...
	CTRL_START[i++] = 0x01; // Indica início // 0x01****
	...
	do {
		...
		DATA_PACK[0] = 0x00; // Indica dados // 0x00****
	...
	CTRL_STOP[0] = 0x02; // Indica paragem // 0x02****
...
int linkwrite(unsigned char* data, int datasize, int Ns) {
	...
	FRAME_I[0] = FLAG;
	FRAME_I[1] = A;
	FRAME_I[2] = Ns == 0 ? 0x00 : 0x20; // 0x40 -> 0x20****
	...
	while (tentativaEnvio <= structDados.numTransmissions) {
		...
		if (res != 0) {
			...
			case 1:
				...
				break;
			case 2:
				if (c == (Ns == 0 ? 0x21 : 0x01)) { // 0x85 -> 0x21; 0x05 -> 0x01****
					estado = 3;
					printf("Confirmation: positive. Switching to state 3\n");
				}
				else if (c == 0x05 || c == 0x25) { // 0x01 -> 0x05; 0x81 -> 0x25****
					podeEnviar = TRUE;
					estado = 0;
				...
				break;
			case 3:
				if (c == (A ^ (Ns == 0 ? 0x21 : 0x01))) { // 0x85 -> 0x21; 0x05 -> 0x01****
					estado = 4;
...
int llread() {
	...
	do {
		...
		if (packageSize == -1) {
			REJ[2] = (Nr == 0) ? 0x05 : 0x25; // 0x01 -> 0x05; 0x81 -> 0x25****
			REJ[3] = REJ[1] ^ REJ[2];
			tcflush(structDados.fd, TCIFLUSH);
			write(structDados.fd, REJ, 5);
		}
		else if (dataPackage[0] == 0x00) { // 0x00****
			int posPackage = 0;
			...
			duplicate = FALSE;
			RR[2] = (Nr == 0) ? 0x01 : 0x21; // 0x05 -> 0x01; 0x85 -> 0x21****
			RR[3] = RR[1] ^ RR[2];
			write(structDados.fd, RR, 5);
		}
		else if (dataPackage[0] == 0x01) { // 0x01****
			unsigned int i = 1;
			unsigned int j;
		...
		else if (dataPackage[0] == 0x02) { // 0x02****
			Nr = (Nr + 1) % 2;
			RR[2] = (Nr == 0) ? 0x01 : 0x21; // 0x05 -> 0x01; 0x85 -> 0x21****
			RR[3] = RR[1] ^ RR[2];
			write(structDados.fd, RR, 5);
...
int linkread(unsigned char* dataPackage) {
	...
	if (Controlo == 0x00) { // Se foi pacote de dados... // 0x00****
